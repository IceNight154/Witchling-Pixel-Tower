...
public class Aria_Harmony extends ArmorAbility {
	...
	public class WndUseTrinity extends WndTitledMessage {
		...
		public WndUseTrinity(ClassArmor armor) {
			super(new HeroIcon(Aria_Harmony.this),
					Messages.titleCase(Aria_Harmony.this.name()),
					...
					btnBody = new RedButton(Messages.get(WndUseTrinity.class, "body",
						...
						protected void onClick() {
							if (Dungeon.hero.belongings.weapon() != null &&
								...
								GLog.w(Messages.get(Aria_Harmony.class, "no_duplicate"));
								hide();
							} else {
	...
	public Talent[] talents() {
		return new Talent[]{Talent.ELEMENTAL_CHORD, Talent.ROTATIONAL_MASTERY, Talent.CONFLUENCE};
	}
			...
			RedButton btnConfirm = new RedButton(text){
				...
				protected void onClick() {
					...
					if (item instanceof MeleeWeapon) {
						((Aria_Harmony)Dungeon.hero.armorAbility).bodyForm = ((MeleeWeapon) item).enchantment;
					} else if (item instanceof Armor) {
						((Aria_Harmony)Dungeon.hero.armorAbility).bodyForm = ((Armor) item).glyph;
					} else if (item instanceof Wand || item instanceof MissileWeapon){
						((Aria_Harmony)Dungeon.hero.armorAbility).mindForm = item;
					} else {
						((Aria_Harmony)Dungeon.hero.armorAbility).spiritForm = item;
					}
	...
	public static String trinityItemUseText(Class<?> cls ){
		...
		if (Weapon.Enchantment.class.isAssignableFrom(cls) || Armor.Glyph.class.isAssignableFrom(cls)) {
			for (Class ench : Weapon.Enchantment.rare) {
				if (ench.equals(cls)) {
					return Messages.get(Aria_Harmony.class, "rare_ench_glyph_use", BodyForm.duration(), Messages.decimalFormat("#.##", chargeUse));
				}
			}
			...
			return Messages.get(Aria_Harmony.class, "ench_glyph_use", BodyForm.duration(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (MissileWeapon.class.isAssignableFrom(cls)){
			return Messages.get(Aria_Harmony.class, "thrown_use", MindForm.itemLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (Wand.class.isAssignableFrom(cls)){
			if (cls.equals(WandOfFireblast.class) || cls.equals(WandOfRegrowth.class)){
				return Messages.get(Aria_Harmony.class, "wand_multi_use", MindForm.itemLevel(), Messages.decimalFormat("#.##", chargeUse));
			}
			return Messages.get(Aria_Harmony.class, "wand_use", MindForm.itemLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (Ring.class.isAssignableFrom(cls)){
			return Messages.get(Aria_Harmony.class, "ring_use", SpiritForm.ringLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (Artifact.class.isAssignableFrom(cls)){
			return Messages.get(Aria_Harmony.class, cls.getSimpleName() + "_use", SpiritForm.artifactLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		return "error!";
