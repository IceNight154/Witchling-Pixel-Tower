...
public class ELEMENTAL_WEAVER extends ArmorAbility {
	...
	public class WndUseTrinity extends WndTitledMessage {
		...
		public WndUseTrinity(ClassArmor armor) {
			super(new HeroIcon(ELEMENTAL_WEAVER.this),
					Messages.titleCase(ELEMENTAL_WEAVER.this.name()),
					...
					btnBody = new RedButton(Messages.get(WndUseTrinity.class, "body",
						...
						protected void onClick() {
							if (Dungeon.hero.belongings.weapon() != null &&
								...
								GLog.w(Messages.get(ELEMENTAL_WEAVER.class, "no_duplicate"));
								hide();
							} else {
	...
	@Override
	public int icon() {
		return HeroIcon.ELEMENTAL_WEAVER;
	}
			...
			RedButton btnConfirm = new RedButton(text){
				...
				protected void onClick() {
					...
					if (item instanceof MeleeWeapon) {
						((ELEMENTAL_WEAVER)Dungeon.hero.armorAbility).bodyForm = ((MeleeWeapon) item).enchantment;
					} else if (item instanceof Armor) {
						((ELEMENTAL_WEAVER)Dungeon.hero.armorAbility).bodyForm = ((Armor) item).glyph;
					} else if (item instanceof Wand || item instanceof MissileWeapon){
						((ELEMENTAL_WEAVER)Dungeon.hero.armorAbility).mindForm = item;
					} else {
						((ELEMENTAL_WEAVER)Dungeon.hero.armorAbility).spiritForm = item;
					}
	...
	public static String trinityItemUseText(Class<?> cls ){
		...
		if (Weapon.Enchantment.class.isAssignableFrom(cls) || Armor.Glyph.class.isAssignableFrom(cls)) {
			for (Class ench : Weapon.Enchantment.rare) {
				if (ench.equals(cls)) {
					return Messages.get(ELEMENTAL_WEAVER.class, "rare_ench_glyph_use", BodyForm.duration(), Messages.decimalFormat("#.##", chargeUse));
				}
			}
			...
			return Messages.get(ELEMENTAL_WEAVER.class, "ench_glyph_use", BodyForm.duration(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (MissileWeapon.class.isAssignableFrom(cls)){
			return Messages.get(ELEMENTAL_WEAVER.class, "thrown_use", MindForm.itemLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (Wand.class.isAssignableFrom(cls)){
			if (cls.equals(WandOfFireblast.class) || cls.equals(WandOfRegrowth.class)){
				return Messages.get(ELEMENTAL_WEAVER.class, "wand_multi_use", MindForm.itemLevel(), Messages.decimalFormat("#.##", chargeUse));
			}
			return Messages.get(ELEMENTAL_WEAVER.class, "wand_use", MindForm.itemLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (Ring.class.isAssignableFrom(cls)){
			return Messages.get(ELEMENTAL_WEAVER.class, "ring_use", SpiritForm.ringLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (Artifact.class.isAssignableFrom(cls)){
			return Messages.get(ELEMENTAL_WEAVER.class, cls.getSimpleName() + "_use", SpiritForm.artifactLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		return "error!";
