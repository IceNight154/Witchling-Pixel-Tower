...
public class ARCHMAGE_OF_HARMONY extends ArmorAbility {
	...
	public class WndUseTrinity extends WndTitledMessage {
		...
		public WndUseTrinity(ClassArmor armor) {
			super(new HeroIcon(ARCHMAGE_OF_HARMONY.this),
					Messages.titleCase(ARCHMAGE_OF_HARMONY.this.name()),
					...
					btnBody = new RedButton(Messages.get(WndUseTrinity.class, "body",
						...
						protected void onClick() {
							if (Dungeon.hero.belongings.weapon() != null &&
								...
								GLog.w(Messages.get(ARCHMAGE_OF_HARMONY.class, "no_duplicate"));
								hide();
							} else {
	...
	@Override
	public int icon() {
		return HeroIcon.ARCHMAGE_OF_HARMONY;
	}
			...
			RedButton btnConfirm = new RedButton(text){
				...
				protected void onClick() {
					...
					if (item instanceof MeleeWeapon) {
						((ARCHMAGE_OF_HARMONY)Dungeon.hero.armorAbility).bodyForm = ((MeleeWeapon) item).enchantment;
					} else if (item instanceof Armor) {
						((ARCHMAGE_OF_HARMONY)Dungeon.hero.armorAbility).bodyForm = ((Armor) item).glyph;
					} else if (item instanceof Wand || item instanceof MissileWeapon){
						((ARCHMAGE_OF_HARMONY)Dungeon.hero.armorAbility).mindForm = item;
					} else {
						((ARCHMAGE_OF_HARMONY)Dungeon.hero.armorAbility).spiritForm = item;
					}
	...
	public static String trinityItemUseText(Class<?> cls ){
		...
		if (Weapon.Enchantment.class.isAssignableFrom(cls) || Armor.Glyph.class.isAssignableFrom(cls)) {
			for (Class ench : Weapon.Enchantment.rare) {
				if (ench.equals(cls)) {
					return Messages.get(ARCHMAGE_OF_HARMONY.class, "rare_ench_glyph_use", BodyForm.duration(), Messages.decimalFormat("#.##", chargeUse));
				}
			}
			...
			return Messages.get(ARCHMAGE_OF_HARMONY.class, "ench_glyph_use", BodyForm.duration(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (MissileWeapon.class.isAssignableFrom(cls)){
			return Messages.get(ARCHMAGE_OF_HARMONY.class, "thrown_use", MindForm.itemLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (Wand.class.isAssignableFrom(cls)){
			if (cls.equals(WandOfFireblast.class) || cls.equals(WandOfRegrowth.class)){
				return Messages.get(ARCHMAGE_OF_HARMONY.class, "wand_multi_use", MindForm.itemLevel(), Messages.decimalFormat("#.##", chargeUse));
			}
			return Messages.get(ARCHMAGE_OF_HARMONY.class, "wand_use", MindForm.itemLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (Ring.class.isAssignableFrom(cls)){
			return Messages.get(ARCHMAGE_OF_HARMONY.class, "ring_use", SpiritForm.ringLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		if (Artifact.class.isAssignableFrom(cls)){
			return Messages.get(ARCHMAGE_OF_HARMONY.class, cls.getSimpleName() + "_use", SpiritForm.artifactLevel(), Messages.decimalFormat("#.##", chargeUse));
		}
		return "error!";
